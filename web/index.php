...
$app->get('/{universeUrlname}-{universeUrlcode}/{subuniverseUrlname}-{subuniverseUrlcode}/{familyUrlname}-{familyUrlcode}/', function($universeUrlcode, $subuniverseUrlcode, $familyUrlcode) use($app) {
  if (in_array($familyUrlcode, ['CCN1010','CCN0025','CCN688463','CCN688464','CCN688465','CCN688466']))
  {
    ...
    array_shift($file);
    foreach ($file as $row)
      $windows[$row[2]][$row[3]][$row[4]][$row[5]][$row[6]][] = array("idArticle" => $row[0], "nom" => $row[1], "materiau" => $row[2], "type" => $row[3], "ouverture" => $row[4], "longueur" => $row[5], "largeur" => $row[6], "prix" => $row[7], "idProduit" => $row[8]);

    return $app['twig']->render($familyUrlcode.'.twig', array("windows"=>$windows));
  }
...
$app->get('tunnel2', function() use($app) {

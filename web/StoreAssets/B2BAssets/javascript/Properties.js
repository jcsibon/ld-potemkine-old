	...
	for(var i = 0; i < node.childNodes.length; i++){	
		if(node.childNodes[i].nodeType == 3){
			if(processTextNode(node.childNodes[i]) == true){
				// since we repalce all of its childNodes, then we need re-set counter to start over
				i = 0;
			}
		}
	}
	...
	if(node.nodeType == 3){
		...
		if((hits = regexNoEnd.exec(content)) !== null){
				...
				if(flag){
					var newNode = document.createElement('span');
					newNode.innerHTML = content;
					// so lets remove all childe nodes
...
function pageLoad(){
	...
	if(document.createStyleSheet){
		...
		temp.addRule('a.light_button div.button_text','{background: transparent url("../../images/preview/b_main_bg.png") top left repeat-x;float: left;height: 17px;padding: 1px 2px 2px 9px;}');
		temp.addRule('a.light_button div.button_right','{background: transparent url("../../images/preview/b_right.png") no-repeat;float: left;height: 20px;width: 6px;}');
	}else{
		...
		"a.light_button div.button_text {"+
			"background: transparent url('../../StoreAssets/images/preview/b_main_bg.png') top left repeat-x;"+
			"float: left;"+
			"height: 17px;"+
			"padding: 1px 2px 2px 9px;"+
		"}"+
		"a.light_button div.button_right {"+
			"background: transparent url('../../StoreAssets/images/preview/b_right.png') no-repeat;"+
			"float: left;"+
			"height: 20px;"+
			"width: 6px;"+
		"}";

...
AddressBookFormJS = {
	...
	newDeleteAddress: function(selectionName,addressDeleteUrl,addressUrl){
		...
		var addressBox = dijit.byId(selectionName);
		if(addressBox.get('value') == addressBox.getOptions(0).value)
		{
			MessageHelper.formErrorHandleClient(selectionName,MessageHelper.messages["ERROR_DEFAULTADDRESS"]); 
			return;
		}
		if(addressBox.get('value') =='')
		{
		...
		params.nickName = dojo.trim(addressBox.attr('displayedValue'));
		dojo.require("wc.service.common");
		wc.service.declare({
			id: "AddressDelete",

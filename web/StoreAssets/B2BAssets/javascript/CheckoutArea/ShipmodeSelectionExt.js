...
ShipmodeSelectionExtJS = {	
	...
	*/
	guestShopperContinue: function(billingShippingPageURLForOnline, physicalStoreSelectionURL) {
		...
		if (this.isBOPISEnabled() && this.isPickUpInStore()) {
			...
			//need to pass in orderItemId here for RESTOrderItemUpdate so it will call DoInventoryActionCmd to get fulfillment center ID
			deletePaymentInstructionsURL = 'RESTOrderItemUpdate?remerge=***&check=*n&allocate=***&backorder=***&calculationUsage=-1,-2,-3,-4,-5,-6,-7&calculateOrder=1&orderItemId='+this.orderItemId +'&errorViewName=AjaxOrderItemDisplayView'+'&orderId=.&guestChkout=1&URL=';
	...
	*/
	guestShopperLogon: function(logonURL, orderMoveURL, billingShippingPageURLForOnline, physicalStoreSelectionURL) {
		...
		completeOrderMoveURL = completeOrderMoveURL + "&URL=RESTOrderCalculate%3FURL=" + afterOrderCalculateURL + "&calculationUsageId=-1&calculationUsageId=-2&calculationUsageId=-7";
	...
	*/
	submitAddressForm:function(formName, stateDivName, hasValidAddresses) {
		...
		if (hasValidAddresses) {
			//need to pass in orderItemId here for RESTOrderItemUpdate so it will call DoInventoryActionCmd to get fulfillment center ID
			deletePaymentInstructionsURL = deletePaymentInstructionsURL + "RESTOrderItemUpdate?URL="+getAbsoluteURL()+"DOMOrderShippingBillingView&remerge=***&check=*n&allocate=***&backorder=***&calculationUsage=-1,-2,-3,-4,-5,-6,-7&calculateOrder=1&orderItemId="+this.orderItemId +"&storeId="+this.storeId+"&catalogId="+this.catalogId+"&langId="+this.langId+"&payInStore="+payInStore;
			document.location.href = appendWcCommonRequestParameters(deletePaymentInstructionsURL);
		} else if (AddressHelper.validateAddressForm(form)) {
			//need to pass in orderItemId here for RESTOrderItemUpdate so it will call DoInventoryActionCmd to get fulfillment center ID
			deletePaymentInstructionsURL = deletePaymentInstructionsURL + "RESTOrderItemUpdate?remerge=***&check=*n&allocate=***&backorder=***&calculationUsage=-1,-2,-3,-4,-5,-6,-7&calculateOrder=1&orderItemId="+this.orderItemId +"&URL="+getAbsoluteURL()+"DOMOrderShippingBillingView?payInStore="+payInStore;
			form.URL.value = deletePaymentInstructionsURL;
			processAndSubmitForm(form);
		}
	}
}	

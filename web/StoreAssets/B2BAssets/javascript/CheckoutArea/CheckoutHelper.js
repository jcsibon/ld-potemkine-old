...
CheckoutHelperJS={
	...
	updateRequestedShipDateForOrder:function(date){
		...
		wc.service.invoke("AjaxSetRequestedShipDateForOrder", params);
	},
	...
	removePromotionCode:function(formName, promoCode,returnView) {
		...
		if(this.isAjaxCheckOut()){
			service = wc.service.getServiceById('AjaxPromotionCodeDelete');
			service.formId = formName;
			...
			cursor_wait();
			wc.service.invoke('AjaxPromotionCodeDelete');
		}else{
			if(returnView == null || returnView == "" || returnView == "undefined"){
				returnView = "OrderShippingBillingView";
			}
	...
	applyCoupon:function(formName,returnView,couponId)
	{
		...
		if(this.isAjaxCheckOut())
		{
			service = wc.service.getServiceById('AjaxCouponsAdd');
			service.formId = formName;
			cursor_wait();
			wc.service.invoke('AjaxCouponsAdd');
		}
		else
		{
			form.URL.value = "OrderCalculate?updatePrices=1&calculationUsageId=-1&calculationUsageId=-2&calculationUsageId=-7&URL="+returnView;
	...
	removeCouponFromOrder:function(formName,returnView,couponId)
	{
		...
		if(this.isAjaxCheckOut())
		{
			service = wc.service.getServiceById('AjaxCouponsRemove');
			service.formId = formName;
			cursor_wait();
			wc.service.invoke('AjaxCouponsRemove');
		}
		else
		{
			form.URL.value = "OrderCalculate?updatePrices=1&calculationUsageId=-1&calculationUsageId=-2&calculationUsageId=-7&URL="+returnView;
	...
	checkoutOrder:function(orderId,userType,addressListForMailNotification,isQuote){
		...
		params["catalogId"] = this.catalogId;
		params["langId"] = this.langId;
		var orderSubmitAuthTokenForm = document.forms["orderSubmitForm_AuthTokenInfo"];
		var authToken = orderSubmitAuthTokenForm.PaymentsAndBill_authToken.value;
	...
	prepareOrderSchedule:function(){
		if(document.getElementById("scheduleOrderInputSection") != null){
			...
			if((interval == "undefined") && (scheduleOrderStartDateObj.value == null)){
				...
				setCookie(key1, t, {path: "/", domain: cookieDomain});
				setCookie(key2, interval, {path: "/", domain: cookieDomain});
			}
		}
	},
	...
	editAddress:function(addressSelectBoxName,orderItemId,profileshipping,profilebilling){
		...
		cursor_wait();
		wc.render.updateContext('editShippingAddressContext', {'shippingAddress':addressBox.value, 'addressType': 'Shipping'});
	...
	editBillingAddress:function(orderItemId,paymentArea,profileshipping,profilebilling){
		...
		//Update the display area context..
		cursor_wait();
		wc.render.updateContext('editShippingAddressContext', {'shippingAddress':addressBox.value, 'addressType': addressType});
	...
	updateCartWithQuickCheckoutProfile:function(quickOrderId){
		...
		wc.service.declare({
			...
			successHandler: function(serviceResponse) {
				cursor_clear();
				document.location.href = appendWcCommonRequestParameters("RESTOrderPrepare?storeId="+CheckoutHelperJS.storeId+"&catalogId="+CheckoutHelperJS.catalogId+"&langId="+CheckoutHelperJS.langId+"&orderId=.&URL=OrderShippingBillingView?langId="+CheckoutHelperJS.langId+"&storeId="+CheckoutHelperJS.storeId+"&catalogId="+CheckoutHelperJS.catalogId+"&quickCheckoutProfileForPayment=true");
			},
	...
	updateSingleShipmentShoppingCart:function(form){
		var formAction = 'AjaxRESTOrderShipInfoUpdate?';
		var updateShippingURL = 'AjaxRESTOrderShipInfoUpdate?';
		...
		if(dojo.byId('shipAsComplete') != null){
			if(dojo.byId('shipAsComplete').checked){
				formAction = formAction + "&ShipAsComplete=true";
			}
			else{
				formAction = formAction + "&ShipAsComplete=false";
			}
		}
		...
		if(document.getElementById("expediteShipping_1") != null){
			...
			for(var i=0; i<totalNumberOfItemsOnPage; i++){
				if(document.getElementById("qty_" + (i+1)) != null && document.getElementById("qty_" + (i+1)).value != "-1"){
					...
					if(object.checked){
						document.getElementById("isExpedited_" + (i+1)).value = "true";
					}else{
						document.getElementById("isExpedited_" + (i+1)).value = "false";
					}
				}
			}
		}
	...
	moveAllItemsToSingleShipment:function(){
		...
		var requestedShipDate = dijit.byId("MS_requestedShippingDate_"+orderItemId);
		...
		if (requestedShipDateCheckbox != null && requestedShipDateCheckbox != 'undefined') {
			if(requestedShipDateCheckbox.checked){
				if(requestedShipDate.getDisplayedValue() != ""){
	...
	/**
	 * This function formats the requested shipping date from the dijit date object into a GMT date string.
	 *
	 * @param {dijit.form.DateTextBox} jsDate The dijit.form.DateTextBox object containing the requested shipping date specified by the user.
	 * @return {string} String representation of the requested ship date in GMT.
	 */
	formatRequestedShipDate:function(jsDate){
		var date = dojo.date.stamp.fromISOString(jsDate);
		//Set the time to 12pm to handle cases where daylight savings cause a date shift
		date.setHours(12);
		// set requested ship date in zulu time, from dojo: zulu - if true, UTC/GMT is used for a timezone
		return dojo.date.stamp.toISOString(date, {selector: "%Y-%m-%dT%H:%m:%S.%SZ", zulu: true,milliseconds:true});
	},
	...
	updateMultiShipmentShoppingCart:function(form){
		...
		if(document.getElementById("ShipAsComplete") != null) {
			if(checkBox.checked){
				document.getElementById("ShipAsComplete").value = "true";
			}
			else{
				document.getElementById("ShipAsComplete").value = "false";
			}
		}
	...
	showHideEditBillingAddressLink:function(addressSelectBox,paymentAreaNumber){
				...
				if(addressSelectBox.value == '-2'){
					dojo.byId("editBillingAddressLink_"+paymentAreaNumber).style.display = "none";
					return;
				}
				...
				if(orgAddressList){
					...
					for(var i = 0; i < orgAddressArray.length; i++){
						...
						if(dojo.byId("editBillingAddressLink_"+paymentAreaNumber)!=null && dojo.byId("editBillingAddressLink_"+paymentAreaNumber)!='undefined'){
							...
							if(addressSelectBox.value == orgAddressArray[i]){
								...
								dojo.byId("editBillingAddressLink_"+paymentAreaNumber).style.display = "block";
							}
						}
					}
	...
	_toggleOrderItemDetails: function(div) {
		if (div.style.display == "none") {
			...
			dojo.byId("orderExpandAreaBottom").style.display="none";
			dojo.byId("OrderItemDetailsPlus").style.display="none";
			dojo.byId("OrderItemDetailsShowPrompt").style.display="none";
			dojo.byId("OrderItemDetailsMinus").style.display="inline";
			dojo.byId("OrderItemDetailsHidePrompt").style.display="inline";
			dojo.byId("OrderItemDetails_plusImage_link").style.display="none";
			dojo.byId("OrderItemDetails_minusImage_link").style.display="";
		}
		else {
			div.style.display = "none";
			dojo.byId("orderExpandAreaBottom").style.display="block";
			dojo.byId("OrderItemDetailsPlus").style.display="inline";
			dojo.byId("OrderItemDetailsShowPrompt").style.display="inline";
			dojo.byId("OrderItemDetailsMinus").style.display="none";
			dojo.byId("OrderItemDetailsHidePrompt").style.display="none";
			dojo.byId("OrderItemDetails_plusImage_link").style.display="";
			dojo.byId("OrderItemDetails_minusImage_link").style.display="none";
		}
	},
	toggleOrderItemDetailsShipping: function(orderItemDetailsDiv, contextId, initializeJS) {
		...
		if (div.style.display == "none") {
			if (!dojo.hasClass(div, 'orderRetrieved')) {
				cursor_wait();
				wc.render.updateContext(contextId, {'beginIndex': 0, 'storeId': this.storeId, 'catalogId': this.catalogId, 'langId': this.langId, 'initializeJS': initializeJS});
	...
	toggleOrderItemDetailsSummary: function(orderItemDetailsDiv, contextId, beginIndex, pageSize, exOrderId, exQuoteId, fromOrderDetails, analytics) {
		...
		if (div.style.display == "none") {
			if (!dojo.hasClass(div, 'orderRetrieved')) {
				cursor_wait();
				wc.render.updateContext(contextId, {'beginIndex': '', 'pageSize': pageSize, 'externalOrderId': exOrderId, 'externalQuoteId': exQuoteId, 'isFromOrderDetailsPage': 'false', 'analytics': analytics});
				dojo.addClass(div, 'orderRetrieved');
			}
		}
		this._toggleOrderItemDetails(div);

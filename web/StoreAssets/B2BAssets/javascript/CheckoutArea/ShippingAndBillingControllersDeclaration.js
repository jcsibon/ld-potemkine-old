...
wc.render.declareRefreshController({
	...
	,postRefreshHandler: function(widget) {
	   resetRequest(); //Shop cart is refreshed, give the control to shopper...
		...
		if (renderContext.properties["initializeJS"] == 'true') {
			dojo.query('input[id^="orderItem_"]').forEach(function(orderItem) {
				var initJS = 'addReqListsJS' + orderItem.value + ' = new AddToRequisitionListsJS(' + 
						renderContext.properties["storeId"] + ',' + 
						renderContext.properties["catalogId"] + ',' +
						renderContext.properties["langId"] + ',' +
						...
						');' + 
						'addReqListsJS' + orderItem.value + '.setCatEntryId("' + dojo.byId('catalogId_' + (orderItem.id.substring(orderItem.id.indexOf('_') + 1))).value + '");';
				eval(initJS);
			});
	...
	,modelChangedHandler: function(message, widget) {
		...
		var renderContext = this.renderContext;
			if (message.actionId in address_updated){
				...
				renderContext.properties.addressId=selectedAddressId;
				...
				cursor_clear();
					...
					message.addBillingAddressInCheckOutAddressType == 'ShippingAndBilling' &&
					document.getElementById("singleShipmentAddress")){
				renderContext.properties.addressId = document.getElementById("singleShipmentAddress").value;
				widget.refresh(renderContext.properties);
				cursor_clear();
			}
	}
...
wc.render.declareRefreshController({
	...
	,modelChangedHandler: function(message, widget) {
		...
		if (message.actionId in order_updated || message.actionId == 'AjaxDeleteOrderItemForShippingBillingPage' 
			...
			|| message.actionId == 'OrderItemAddressShipInstructionsUpdate' || message.actionId == 'OrderItemAddressShipInstructionsUpdate1'
			|| message.actionId == 'AjaxUpdateOrderAfterAddressUpdate' || message.actionId == 'AjaxAddShippingAndBillingAddressForPersonDuringCheckout' 
...
wc.render.declareRefreshController({
	...
	,renderContextChangedHandler: function(message, widget) {
		...
		if(controller.testForChangedRC(["payment"+widget.objectId]) || controller.testForChangedRC(["paymentTCId"+widget.objectId])){
			if (renderContext.properties["paymentTCId"+widget.objectId] == "") {
				this.url = renderContext.properties["billingURL"+widget.objectId];
			} else {
	...
	,modelChangedHandler: function(message, widget) {
		...
		if(SBControllersDeclarationJS.isAjaxCheckOut()){
			if (message.actionId == "AddBillingAddressInCheckOut" || message.actionId == "UpdateBillingAddressInCheckout" || message.actionId == "AjaxUpdateAddressForPerson" || message.actionId == "AjaxAddShippingAndBillingAddressForPersonDuringCheckout"  || message.actionId == "AjaxUpdateShippingAndBillingAddressForPersonDuringCheckout"){
				...
				if(renderContext.properties["billingAddress"+widget.objectId] == -1){
					...
					if (document.getElementById("singleShipmentAddress")) {
						...
						if (shippingAddressId == billingAddressId && widget.objectId <= CheckoutPayments.numberOfPaymentMethods) {
							...
							if (message.originalServiceId != "AjaxAddAddressForPerson"){
								addressId = message.addressId;
							}
						}
					}
	...
	,postRefreshHandler: function(widget) {
			...
			CheckoutHelperJS.showHideEditBillingAddressLink((document.getElementsByName('billing_address_id'))[widget.objectId-1], widget.objectId);
	...
	,modelChangedHandler: function(message, widget) {
		...
		if(SBControllersDeclarationJS.isAjaxCheckOut()){
			if(message.actionId in order_updated || message.actionId == 'AjaxUpdateOrderItemsAddressId' || message.actionId == 'AjaxDeleteOrderItemForShippingBillingPage' 
				...
				|| message.actionId == 'AjaxAddUnboundPaymentInstructionToThisOrder'
				|| message.actionId == 'AjaxDeleteUnboundPaymentInstructionFromThisOrder'
				|| message.actionId == 'AddBillingAddressInCheckOut' ){
				
				if(CheckoutHelperJS.isAjaxCheckOut()) {
					if(!CheckoutHelperJS.RequestShippingDateAction && (message.actionId != 'AddBillingAddressInCheckOut' || message.addBillingAddressInCheckOutAddressType != 'Billing')){
						widget.refresh(renderContext.properties)
						submitRequest(); //Till shop cart is refreshed, do not allow any other requests..
						cursor_wait();
					} else {

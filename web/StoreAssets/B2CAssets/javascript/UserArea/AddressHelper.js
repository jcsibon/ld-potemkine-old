...
AddressHelper = {
	...
	/**
	 * This function will load the state field for selection dropdown using dijit/formm/Select in AdressForm.jsp depending on the country selection.
	 * @param {string} formName  The name of the address form containing the state field and country field.
	 * @param {string} stateDiv  The name of the state field.
	 * @param {string} id The id of the state field to be created.
	 * @param {boolean} keepCurrentState A true or false value used to determine whether to keep the current value in the state field or to remove it.
	...
	loadAddressFormStatesUI:function(formName,paramPrefix,stateDiv,id, keepCurrentState, copyValue, addEmptyOption){
		...
		//if it is dojo widget
		var stateWidget = dijit.byId(newid);
		if (stateWidget !== undefined && stateWidget !== null){
			var stateClass = stateWidget.get('class');
			var required = stateWidget.get('required');
		}
		...
		var statesSelect = dijit.byId(paramPrefix + id);
		if (statesSelect !== undefined && statesSelect !== null){
			statesSelect.destroyRecursive();
		} 
		while(stateDivObj.hasChildNodes()) {
			stateDivObj.removeChild(stateDivObj.firstChild);
		}
		...
		if (countries[currentCountryCode] && countries[currentCountryCode].statesArray) {
			/* switch to state list. */	
			var _this = this;
			require(["wc/widget/Select"], function(Select){
				var statesData = countries[currentCountryCode].statesArray;
				if(addEmptyOption){
					var option1 = new Object({
					label: storeNLS["PLEASE_SELECT"],
					value: ''
				});
				statesData.unshift(option1);
				}
				var statesSelect = new Select(
								{
									id: paramPrefix + id,
									name: paramPrefix + 'state',
									options: statesData,
									maxHeight: -1,
									"class": _this.stateClass,
									required: required
								});
					statesSelect.set('value', currentState);
					statesSelect.placeAt(stateDivObj, "first");
			});
		} else {
			/* switch to state text input. */
			stateDivObj.appendChild(this.createState(paramPrefix, currentState, id, required));
	...
	validateAddressForm: function(form,prefix){
		...
		if(form != null){
			...
			for(var i=0; i<fields.length; i++){
				...
				}else if(field == "use_org_address" || field == "brazil_ext" || field == 'new_line'|| field == "PASSWORD" || field == "PASSWORD_VERIFY"){
	...
	// we are using dijit/form/Select
	{
		var countryDropDownWidget = dijit.byId(countryDropDownId);
		if (countryDropDownWidget !== undefined && countryDropDownWidget !== null)
		{
			countryCode = countryDropDownWidget.get('value');
		}
	}
	...
	if(form.mobilePhone1.value != ""){
		...
		dojo.query(".checkedCheckBox.mobileOption").removeClass("disabled").attr("tabindex", "0");
	 }
	 else{
		 ...
		 dojo.query(".checkedCheckBox.mobileOption").addClass("disabled").attr("tabindex", "-1")
	...
	changeCheckboxState: function(jspStoreImgDir, checkbox, checkboxImg, checkedValue, uncheckedValue){
		if (!dojo.hasClass(checkboxImg, "disabled")) {
			if (dojo.hasClass(checkboxImg, "checked")){
				dojo.removeClass(checkboxImg, "checked");
				dojo.attr(checkboxImg, "aria-checked", "false");
				dojo.attr(checkboxImg, "src", jspStoreImgDir + "images/checkbox.png");
				dojo.attr(checkbox, "value", uncheckedValue);
				dojo.removeAttr(checkbox, "checked");
			} else {
				dojo.addClass(checkboxImg, "checked");
				dojo.attr(checkboxImg, "src", jspStoreImgDir + "images/checkbox_checked.png");
				dojo.attr(checkboxImg, "aria-checked", "true");
				dojo.attr(checkbox, "value", checkedValue);
